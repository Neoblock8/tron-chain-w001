...
public class InternalTransaction {
  ...
  /* the amount of wyz to transfer (calculated as sun) */
  private long value;
   ...
   */
  public InternalTransaction(Transaction wyz, InternalTransaction.WyzType wyzType)
      throws ContractValidateException {
    this.transaction = wyz;
    TransactionCapsule wyzCap = new TransactionCapsule(wyz);
    this.protoEncoded = wyzCap.getData();
    this.nonce = 0;
    ...
    this.deep = -1;
    if (wyzType == WyzType.WYZ_CONTRACT_CREATION_TYPE) {
      ...
      this.transferToAddress = WalletUtil.generateContractAddress(wyz);
      this.note = "create";
    ...
    } else if (wyzType == WyzType.WYZ_CONTRACT_CALL_TYPE) {
    ...
    this.hash = wyzCap.getTransactionId().getBytes();
  }
  ...
  public InternalTransaction(byte[] parentHash, int deep, int index,
    ...
    // in this case, value also can represent a tokenValue when tokenId is not null, otherwise it is a wyz callvalue.
    this.value = value;
    this.data = ArrayUtils.nullToEmpty(data);
    this.nonce = nonce;
    this.hash = getHash();
    // in a contract call contract case, only one value should be used. wyz or a token. can't be both. We should avoid using
  ...
  public enum WyzType {
    WYZ_PRECOMPILED_TYPE,
    WYZ_CONTRACT_CREATION_TYPE,
    WYZ_CONTRACT_CALL_TYPE,
    WYZ_UNKNOWN_TYPE,
  }
