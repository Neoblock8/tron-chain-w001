...
public class StorageMarket {
  ...
  private long exchangeToSupply(boolean isWYZ, long quant) {
    logger.info("isWYZ: " + isWYZ);
    long balance = isWYZ ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchangeToSupply2(boolean isWYZ, long quant) {
    logger.info("isWYZ: " + isWYZ);
    long balance = isWYZ ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isWYZ, long supplyQuant) {
    long balance = isWYZ ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isWYZ) {
    long relay = exchangeToSupply(isWYZ, from);
    return exchange_from_supply(!isWYZ, relay);
  }
