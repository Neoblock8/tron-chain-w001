...
import static org.tron.common.runtime.InternalTransaction.WyzType.WYZ_CONTRACT_CALL_TYPE;
import static org.tron.common.runtime.InternalTransaction.WyzType.WYZ_CONTRACT_CREATION_TYPE;
...
public class TransactionTrace {
  ...
  public TransactionTrace(TransactionCapsule wyz, StoreFactory storeFactory,
      Runtime runtime) {
    this.wyz = wyz;
    Transaction.Contract.ContractType contractType = this.wyz.getInstance().getRawData()
        .getContract(0).getType();
    switch (contractType.getNumber()) {
      case ContractType.TriggerSmartContract_VALUE:
        wyzType = WYZ_CONTRACT_CALL_TYPE;
        break;
      case ContractType.CreateSmartContract_VALUE:
        wyzType = WYZ_CONTRACT_CREATION_TYPE;
        break;
      default:
        wyzType = WyzType.WYZ_PRECOMPILED_TYPE;
    }
  ...
  public TransactionCapsule getWyz() {
    return wyz;
  }

  private boolean needVM() {
    return this.wyzType == WYZ_CONTRACT_CALL_TYPE
        || this.wyzType == WYZ_CONTRACT_CREATION_TYPE;
  }
  ...
  public void init(BlockCapsule blockCap, boolean eventPluginLoaded) {
    ...
    transactionContext = new TransactionContext(blockCap, wyz, storeFactory, false,
        eventPluginLoaded);
  }

  public void checkIsConstant() throws ContractValidateException, VMIllegalException {
    if (dynamicPropertiesStore.getAllowWvmConstantinople() == 1) {
      return;
    }
    TriggerSmartContract triggerContractFromTransaction = ContractCapsule
        .getTriggerContractFromTransaction(this.getWyz().getInstance());
    if (WYZ_CONTRACT_CALL_TYPE == this.wyzType) {
  ...
  public void exec()
    ...
    if (WyzType.WYZ_PRECOMPILED_TYPE != wyzType) {
  ...
  public void pay() throws BalanceInsufficientException {
    ...
    switch (wyzType) {
      case WYZ_CONTRACT_CREATION_TYPE:
        callerAccount = TransactionCapsule.getOwner(wyz.getInstance().getRawData().getContract(0));
        originAccount = callerAccount;
        break;
      case WYZ_CONTRACT_CALL_TYPE:
        TriggerSmartContract callContract = ContractCapsule
            .getTriggerContractFromTransaction(wyz.getInstance());
  ...
  public boolean checkNeedRetry() {
    ...
    return wyz.getContractRet() != contractResult.OUT_OF_TIME && receipt.getResult()
  ...
  public void check() throws ReceiptCheckErrException {
    ...
    if (Objects.isNull(wyz.getContractRet())) {
      throw new ReceiptCheckErrException("null resultCode");
    }
    if (!wyz.getContractRet().equals(receipt.getResult())) {
      logger.info(
          "this tx id: {}, the resultCode in received block: {}, the resultCode in self: {}",
          Hex.toHexString(wyz.getTransactionId().getBytes()), wyz.getContractRet(),
          receipt.getResult());
