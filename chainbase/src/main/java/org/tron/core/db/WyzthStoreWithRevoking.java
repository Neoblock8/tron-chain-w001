...
import org.tron.core.db2.core.IWyzthChainBase;
...
@Slf4j(topic = "DB")
public abstract class WyzthStoreWithRevoking<T extends ProtoCapsule> implements IWyzthChainBase<T> {
  ...
  protected WyzthStoreWithRevoking(String dbName) {
  ...
  protected WyzthStoreWithRevoking(DB<byte[], byte[]> db) {
    int dbVersion = CommonParameter.getInstance().getStorage().getDbVersion();
    if (dbVersion == 2) {
  ...
  // only for test
  protected WyzthStoreWithRevoking(String dbName, RevokingDatabase revokingDatabase) {
    this.revokingDB = new RevokingDBWithCachingOldValue(dbName,
        (AbstractRevokingStore) revokingDatabase);
  }

  // only for test
  protected WyzthStoreWithRevoking(String dbName, Options options,
