...
public class TransactionUtil {
  ...
  public static TransactionInfoCapsule buildTransactionInfoInstance(TransactionCapsule wyzCap,
    ...
    builder.setId(ByteString.copyFrom(wyzCap.getTransactionId().getBytes()));
    ...
    if (CommonParameter.getInstance().isSaveInternalTx() && null != programResult
      ...
      for (InternalTransaction internalTransaction : programResult
          .getInternalTransactions()) {
        Protocol.InternalTransaction.Builder internalWyzBuilder = Protocol.InternalTransaction
            .newBuilder();
        // set hash
        internalWyzBuilder.setHash(ByteString.copyFrom(internalTransaction.getHash()));
        // set caller
        internalWyzBuilder.setCallerAddress(ByteString.copyFrom(internalTransaction.getSender()));
        // set TransferTo
        internalWyzBuilder
        ...
        // wyz will not be set token name
        callValueInfoBuilder.setCallValue(internalTransaction.getValue());
        // Just one transferBuilder for now.
        internalWyzBuilder.addCallValueInfo(callValueInfoBuilder);
        internalTransaction.getTokenInfo().forEach((tokenId, amount) -> {
          internalWyzBuilder.addCallValueInfo(
              Protocol.InternalTransaction.CallValueInfo.newBuilder().setTokenId(tokenId)
                  .setCallValue(amount));
        });
        // Token for loop end here
        internalWyzBuilder.setNote(ByteString.copyFrom(internalTransaction.getNote().getBytes()));
        internalWyzBuilder.setRejected(internalTransaction.isRejected());
        builder.addInternalTransactions(internalWyzBuilder);
      }
    }
