...
public class ContractCapsule implements ProtoCapsule<SmartContract> {
  ...
  public static CreateSmartContract getSmartContractFromTransaction(Transaction wyz) {
    try {
      Any any = wyz.getRawData().getContract(0).getParameter();
  ...
  public static TriggerSmartContract getTriggerContractFromTransaction(Transaction wyz) {
    try {
      Any any = wyz.getRawData().getContract(0).getParameter();
  ...
  public byte[] getWyzHash() {
    return this.smartContract.getWyzHash().toByteArray();
  }
}
