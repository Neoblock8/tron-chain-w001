...
public class VM {
  ...
  public void step(Program program) {
    ...
    try {
      ...
      if (op == null
          || (!VMConfig.allowWvmTransferTrc10()
              && (op == CALLTOKEN || op == TOKENBALANCE || op == CALLTOKENVALUE
          || op == CALLTOKENID))
          || (!VMConfig.allowWvmConstantinople()
              && (op == SHL || op == SHR || op == SAR || op == CREATE2 || op == EXTCODEHASH))
          || (!VMConfig.allowWvmSolidity059() && op == ISCONTRACT)
          || (!VMConfig.allowWvmIstanbul() && (op == SELFBALANCE || op == CHAINID))
          ) {
