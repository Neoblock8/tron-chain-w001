...
public class Storage {
  ...
  private static byte[] addrHash(byte[] address, byte[] wyzHash) {
    if (ByteUtil.isNullOrZeroArray(wyzHash)) {
      return Hash.sha3(address);
    }
    return Hash.sha3(ByteUtil.merge(address, wyzHash));
  }

  public void generateAddrHash(byte[] wyzId) {
    // update addreHash for create2
    addrHash = addrHash(address, wyzId);
  }
