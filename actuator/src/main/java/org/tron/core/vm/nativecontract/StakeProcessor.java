...
public class StakeProcessor {
  ...
  public void process(StakeParam stakeParam, Repository repository)
    ...
    long voteCount = stakeParam.getStakeAmount() / ChainConstant.WYZ_PRECISION;
  ...
  private void validateFreeze(byte[] ownerAddress, long frozenDuration,
    ...
    if (frozenBalance < ChainConstant.WYZ_PRECISION) {
      throw new ContractValidateException("frozenBalance must be more than 1WYZ");
    }
  ...
  private void validateVote(byte[] ownerAddress, Protocol.Vote vote, Repository repository)
    ...
    try {
      ...
      // wyz -> drop. The vote count is based on WYZ
      sum = LongMath.checkedMultiply(sum, ChainConstant.WYZ_PRECISION);
      if (sum > tronPower) {
        throw new ContractValidateException(
  ...
  private void executeFreeze(byte[] ownerAddress, long frozenDuration,
    ...
    repository
            .addTotalNetWeight(frozenBalance / ChainConstant.WYZ_PRECISION);
  }
