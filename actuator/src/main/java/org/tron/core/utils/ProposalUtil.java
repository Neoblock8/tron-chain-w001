...
public class ProposalUtil {
  ...
  public static void validator(DynamicPropertiesStore dynamicPropertiesStore,
    ...
    switch (proposalType) {
      ...
      case ALLOW_WVM_TRANSFER_TRC10: {
        if (value != 1) {
          throw new ContractValidateException(
              PRE_VALUE_NOT_ONE_ERROR + "ALLOW_WVM_TRANSFER_TRC10" + VALUE_NOT_ONE_ERROR);
        }
        if (dynamicPropertiesStore.getAllowSameTokenName() == 0) {
          throw new ContractValidateException("[ALLOW_SAME_TOKEN_NAME] proposal must be approved "
              + "before [ALLOW_WVM_TRANSFER_TRC10] can be proposed");
        }
        break;
      }
      ...
      case ALLOW_WVM_CONSTANTINOPLE: {
        if (!forkController.pass(ForkBlockVersionEnum.VERSION_3_6)) {
          throw new ContractValidateException(BAD_PARAM_ID);
        }
        if (value != 1) {
          throw new ContractValidateException(
              PRE_VALUE_NOT_ONE_ERROR + "ALLOW_WVM_CONSTANTINOPLE" + VALUE_NOT_ONE_ERROR);
        }
        if (dynamicPropertiesStore.getAllowWvmTransferTrc10() == 0) {
          throw new ContractValidateException(
              "[ALLOW_WVM_TRANSFER_TRC10] proposal must be approved "
                  + "before [ALLOW_WVM_CONSTANTINOPLE] can be proposed");
        }
        break;
      }
      case ALLOW_WVM_SOLIDITY_059: {
        ...
        if (value != 1) {
          throw new ContractValidateException(
              PRE_VALUE_NOT_ONE_ERROR + "ALLOW_WVM_SOLIDITY_059" + VALUE_NOT_ONE_ERROR);
        }
        if (dynamicPropertiesStore.getAllowCreationOfContracts() == 0) {
          throw new ContractValidateException(
              "[ALLOW_CREATION_OF_CONTRACTS] proposal must be approved "
                  + "before [ALLOW_WVM_SOLIDITY_059] can be proposed");
        }
        break;
      }
...
//      }
      ...
      case ALLOW_WVM_ISTANBUL: {
        if (!forkController.pass(ForkBlockVersionEnum.VERSION_4_1)) {
          throw new ContractValidateException(
              "Bad chain parameter id [ALLOW_WVM_ISTANBUL]");
        }
        if (value != 1) {
          throw new ContractValidateException(
              "This value[ALLOW_WVM_ISTANBUL] is only allowed to be 1");
        }
        break;
      }
...
//      case ALLOW_WVM_STAKE: {
//          if (!forkController.pass(ForkBlockVersionEnum.VERSION_4_1)) {
//          throw new ContractValidateException(
//              "Bad chain parameter id [ALLOW_WVM_STAKE]");
//        }
//        if (value != 1 && value != 0) {
//          throw new ContractValidateException(
//              "This value[ALLOW_WVM_STAKE] is only allowed to be 1 or 0");
//        }
//        break;
//      }
      //  case ALLOW_WVM_ASSET_ISSUE: {
      //  if (!forkController.pass(ForkBlockVersionEnum.VERSION_4_1)) {
      //      throw new ContractValidateException(
      //          "Bad chain parameter id [ALLOW_WVM_ASSET_ISSUE]");
      //  }
      //  if (value != 1 && value != 0) {
      //    throw new ContractValidateException(
      //        "This value[ALLOW_WVM_ASSET_ISSUE] is only allowed to be 1 or 0");
      //  }
      //  break;
      //}
  ...
  public enum ProposalType {         // current value, value range
    ...
    ACCOUNT_UPGRADE_COST(1), // 9999 WYZ, [0, 100000000000] WYZ
    CREATE_ACCOUNT_FEE(2), // 0.1 WYZ, [0, 100000000000] WYZ
    TRANSACTION_FEE(3), // 10 Sun/Byte, [0, 100000000000] WYZ
    ASSET_ISSUE_FEE(4), // 1024 WYZ, [0, 100000000000] WYZ
    WITNESS_PAY_PER_BLOCK(5), // 16 WYZ, [0, 100000000000] WYZ
    WITNESS_STANDBY_ALLOWANCE(6), // 115200 WYZ, [0, 100000000000] WYZ
    CREATE_NEW_ACCOUNT_FEE_IN_SYSTEM_CONTRACT(7), // 0 WYZ, [0, 100000000000] WYZ
    ...
    ENERGY_FEE(11), // 10 Sun, [0, 100000000000] WYZ
    EXCHANGE_CREATE_FEE(12), // 1024 WYZ, [0, 100000000000] WYZ
