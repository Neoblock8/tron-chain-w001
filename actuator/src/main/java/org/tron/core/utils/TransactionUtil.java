...
public class TransactionUtil {
  ...
  public static TransactionCapsule getTransactionSign(TransactionSign transactionSign) {
    ...
    TransactionCapsule wyz = new TransactionCapsule(transactionSign.getTransaction());
    wyz.sign(privateKey);
    return wyz;
  }
  ...
  public TransactionCapsule addSign(TransactionSign transactionSign)
    ...
    byte[] privateKey = transactionSign.getPrivateKey().toByteArray();
    TransactionCapsule wyz = new TransactionCapsule(transactionSign.getTransaction());
    wyz.addSign(privateKey, chainBaseManager.getAccountStore());
    return wyz;
  }

  public TransactionSignWeight getTransactionSignWeight(Transaction wyz) {
    ...
    try {
      ...
      if (wyz.getSignatureCount() > 0) {
        ...
        long currentWeight = TransactionCapsule.checkWeight(permission, wyz.getSignatureList(),
            Sha256Hash.hash(CommonParameter.getInstance()
                .isECKeyCryptoEngine(), wyz.getRawData().toByteArray()), approveList);
