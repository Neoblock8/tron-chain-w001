...
public class UnfreezeBalanceActuator extends AbstractActuator {
  ...
  public boolean execute(Object result) throws ContractExeException {
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      if (dynamicStore.getAllowWvmConstantinople() == 0 ||
          (receiverCapsule != null && receiverCapsule.getType() != AccountType.Contract)) {
        switch (unfreezeBalanceContract.getResource()) {
          case BANDWIDTH:
            if (dynamicStore.getAllowWvmSolidity059() == 1
    ...
    switch (unfreezeBalanceContract.getResource()) {
      case BANDWIDTH:
        dynamicStore
            .addTotalNetWeight(-unfreezeBalance / WYZ_PRECISION);
        break;
      case ENERGY:
        dynamicStore
            .addTotalEnergyWeight(-unfreezeBalance / WYZ_PRECISION);
        break;
      default:
        //this should never happen
        break;
