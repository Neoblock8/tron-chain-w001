...
import org.tron.common.runtime.InternalTransaction.WyzType;
...
public class VMActuator implements Actuator2 {
  ...
  @Getter
  @Setter
  private InternalTransaction.WyzType wyzType;
  ...
  public void validate(Object object) throws ContractValidateException {
    ...
    wyz = context.getWyzCap().getInstance();
    blockCap = context.getBlockCap();
    //Route Type
    ContractType contractType = this.wyz.getRawData().getContract(0).getType();
    //Prepare Repository
    ...
    switch (contractType.getNumber()) {
      case ContractType.TriggerSmartContract_VALUE:
        wyzType = WyzType.WYZ_CONTRACT_CALL_TYPE;
        call();
        break;
      case ContractType.CreateSmartContract_VALUE:
        wyzType = WyzType.WYZ_CONTRACT_CREATION_TYPE;
        create();
        break;
      default:
    ...
    try {
      if (vm != null) {
        if (null != blockCap && blockCap.generatedByMyself && blockCap.hasWitnessSignature()
            && null != TransactionUtil.getContractRet(wyz)
            && contractResult.OUT_OF_TIME == TransactionUtil.getContractRet(wyz)) {
        ...
        if (isConstantCall) {
          long callValue = TransactionCapsule.getCallValue(wyz.getRawData().getContract(0));
          long callTokenValue = TransactionUtil
              .getCallTokenValue(wyz.getRawData().getContract(0));
        ...
        if (WyzType.WYZ_CONTRACT_CREATION_TYPE == wyzType && !result.isRevert()) {
  ...
  private void create()
    ...
    if (!repository.getDynamicPropertiesStore().supporwvm()) {
      throw new ContractValidateException("vm work is off, need to be opened by the committee");
    }

    CreateSmartContract contract = ContractCapsule.getSmartContractFromTransaction(wyz);
    if (contract == null) {
    ...
    try {
      long feeLimit = wyz.getRawData().getFeeLimit();
      ...
      rootInternalTransaction = new InternalTransaction(wyz, wyzType);
      ...
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(WyzType.WYZ_CONTRACT_CREATION_TYPE, executorType, wyz,
      ...
      byte[] txId = TransactionUtil.getTransactionId(wyz).getBytes();
    ...
    if (VMConfig.allowWvmTransferTrc10() && tokenValue > 0) {
  ...
  private void call()
    ...
    if (isNotEmpty(code)) {
      ...
      long vmShouldEndInUs = vmStartInUs + thisTxCPULimitInUs;
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(WyzType.WYZ_CONTRACT_CALL_TYPE, executorType, wyz,
              tokenValue, tokenId, blockCap.getInstance(), repository, vmStartInUs,
              vmShouldEndInUs, energyLimit);
      if (isConstantCall) {
  ...
  public long getTotalEnergyLimit(AccountCapsule creator, AccountCapsule caller,
      ...
      throws ContractValidateException {
    if (Objects.isNull(creator) && VMConfig.allowWvmConstantinople()) {
  ...
  public void checkTokenValueAndId(long tokenValue, long tokenId) throws ContractValidateException {
    if (VMConfig.allowWvmTransferTrc10() && VMConfig.allowMultiSign()) {
  ...
  private double getCpuLimitInUsRatio() {
    ...
    if (ExecutorType.ET_NORMAL_TYPE == executorType) {
      ...
      if (this.blockCap != null && blockCap.generatedByMyself &&
        ...
        if (wyz.getRet(0).getContractRet() == contractResult.OUT_OF_TIME) {
  ...
  public long getTotalEnergyLimitWithFixRatio(AccountCapsule creator, AccountCapsule caller,
    ...
    if (Arrays.equals(creator.getAddress().toByteArray(), caller.getAddress().toByteArray())) {
      ...
      // which will cost the feeLimit WYZ when the creator's frozen energy is 0.
