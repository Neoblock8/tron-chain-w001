...
public class DepositImpl implements Deposit {
  ...
  public synchronized Storage getStorage(byte[] address) {
    ...
    ContractCapsule contract = getContract(address);
    if (contract != null && !ByteUtil.isNullOrZeroArray(contract.getWyzHash())) {
      storage.generateAddrHash(contract.getWyzHash());
    }
    return storage;
  }
  ...
  @Override
  public TransactionCapsule getTransaction(byte[] wyzHash) {
    Key key = Key.create(wyzHash);
    ...
    if (parent != null) {
      transactionCapsule = parent.getTransaction(wyzHash);
    } else {
      try {
        transactionCapsule = getTransactionStore().get(wyzHash);
