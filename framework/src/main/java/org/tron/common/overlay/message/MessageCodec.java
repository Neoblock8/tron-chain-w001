...
import org.tron.core.net.message.WyzthMessageFactory;
...
public class MessageCodec extends ByteToMessageDecoder {
  ...
  private WyzthMessageFactory WyzthMessageFactory = new WyzthMessageFactory();
  ...
  private Message createMessage(byte[] encoded) throws Exception {
    ...
    if (MessageTypes.inTronRange(type)) {
      return WyzthMessageFactory.create(encoded);
    }
