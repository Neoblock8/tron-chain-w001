...
@Slf4j(topic = "net")
@Component
public class FetchInvDataMsgHandler implements WyzthMsgHandler {
  ...
  public void processMessage(PeerConnection peer, WyzthMessage msg) throws P2pException {
    ...
    for (Sha256Hash hash : fetchInvDataMsg.getHashList()) {
      ...
      if (message == null) {
        try {
          message = WyzthNetDelegate.getData(hash, type);
        } catch (Exception e) {
  ...
  private void sendPbftCommitMessage(PeerConnection peer, BlockCapsule blockCapsule) {
    try {
      if (!WyzthNetDelegate.allowPBFT() || peer.isSyncFinish()) {
        return;
      }
      long epoch = 0;
      PbftSignCapsule pbftSignCapsule = WyzthNetDelegate
      ...
      if (epochCache.getIfPresent(epoch) == null) {
        PbftSignCapsule srl = WyzthNetDelegate.getSRLPbftCommitData(epoch);
        if (srl != null) {
