...
@Slf4j(topic = "net")
@Component
public class SyncBlockChainMsgHandler implements WyzthMsgHandler {
  ...
  public void processMessage(PeerConnection peer, WyzthMessage msg) throws P2pException {
    ...
    if (blockIds.size() == 1) {
      ...
      remainNum = WyzthNetDelegate.getHeadBlockId().getNum() - blockIds.peekLast().getNum();
    }
  ...
  private void check(PeerConnection peer, SyncBlockChainMessage msg) throws P2pException {
    ...
    if (!WyzthNetDelegate.containBlockInMainChain(firstId)) {
      throw new P2pException(TypeEnum.BAD_MESSAGE, "No first block:" + firstId.getString());
    }

    long headNum = WyzthNetDelegate.getHeadBlockId().getNum();
  ...
  private LinkedList<BlockId> getLostBlockIds(List<BlockId> blockIds) throws P2pException {
    ...
    for (int i = blockIds.size() - 1; i >= 0; i--) {
      if (WyzthNetDelegate.containBlockInMainChain(blockIds.get(i))) {
