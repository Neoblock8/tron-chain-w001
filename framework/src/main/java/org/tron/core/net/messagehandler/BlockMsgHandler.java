...
@Slf4j(topic = "net")
@Component
public class BlockMsgHandler implements WyzthMsgHandler {
  ...
  public void processMessage(PeerConnection peer, WyzthMessage msg) throws P2pException {
    ...
    if (peer.getSyncBlockRequested().containsKey(blockId)) {
      ...
      long interval = blockId.getNum() - WyzthNetDelegate.getHeadBlockId().getNum();
  ...
  private void processBlock(PeerConnection peer, BlockCapsule block) throws P2pException {
    ...
    if (!WyzthNetDelegate.containBlock(block.getParentBlockId())) {
      logger.warn("Get unlink block {} from {}, head is {}.", blockId.getString(),
          peer.getInetAddress(), WyzthNetDelegate.getHeadBlockId().getString());
    ...
    if (fastForward) {
      if (block.getNum() < WyzthNetDelegate.getHeadBlockId().getNum()) {
        logger.warn("Receive a low block {}, head {}",
            blockId.getString(), WyzthNetDelegate.getHeadBlockId().getString());
        return;
      }
      if (WyzthNetDelegate.validBlock(block)) {
        advService.fastForward(new BlockMessage(block));
        WyzthNetDelegate.trustNode(peer);
      }
    }

    WyzthNetDelegate.processBlock(block, false);
    witnessProductBlockService.validWitnessProductTwoBlock(block);
    WyzthNetDelegate.getActivePeer().forEach(p -> {
