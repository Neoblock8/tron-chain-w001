...
@Slf4j(topic = "net")
@Component
public class ChainInventoryMsgHandler implements WyzthMsgHandler {
  ...
  public void processMessage(PeerConnection peer, WyzthMessage msg) throws P2pException {
    ...
    if (blockIdWeGet.size() == 1 && WyzthNetDelegate.containBlock(blockIdWeGet.peek())) {
    ...
    synchronized (WyzthNetDelegate.getBlockLock()) {
      while (!peer.getSyncBlockToFetch().isEmpty() && WyzthNetDelegate
  ...
  private void check(PeerConnection peer, ChainInventoryMessage msg) throws P2pException {
    ...
    if (WyzthNetDelegate.getHeadBlockId().getNum() > 0) {
      long maxRemainTime =
          ChainConstant.CLOCK_MAX_DELAY + System.currentTimeMillis() - WyzthNetDelegate
              .getBlockTime(WyzthNetDelegate.getSolidBlockId());
      long maxFutureNum =
          maxRemainTime / BLOCK_PRODUCED_INTERVAL + WyzthNetDelegate.getSolidBlockId().getNum();
