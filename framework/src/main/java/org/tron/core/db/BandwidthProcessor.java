...
public class BandwidthProcessor extends ResourceProcessor {
  ...
  @Override
  public void consume(TransactionCapsule wyz, TransactionTrace trace)
      throws ContractValidateException, AccountResourceInsufficientException,
      TooBigTransactionResultException {
    List<Contract> contracts = wyz.getInstance().getRawData().getContractList();
    if (wyz.getResultSerializedSize() > Constant.MAX_RESULT_SIZE_IN_TX * contracts.size()) {
    ...
    if (chainBaseManager.getDynamicPropertiesStore().supporwvm()) {
      bytesSize = wyz.getInstance().toBuilder().clearRet().build().getSerializedSize();
    } else {
      bytesSize = wyz.getSerializedSize();
    }
    ...
    for (Contract contract : contracts) {
      ...
      logger.debug("wyzId {}, bandwidth cost: {}", wyz.getTransactionId(), bytesSize);
  ...
  public long calculateGlobalNetLimit(AccountCapsule accountCapsule) {
    ...
    if (frozeBalance < WYZ_PRECISION) {
      return 0;
    }
    long netWeight = frozeBalance / WYZ_PRECISION;
