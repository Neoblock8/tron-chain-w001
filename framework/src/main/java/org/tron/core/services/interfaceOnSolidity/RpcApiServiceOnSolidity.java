...
public class RpcApiServiceOnSolidity implements Service {
  ...
  private TransactionExtention transaction2Extention(Transaction transaction) {
    ...
    wyzExtBuilder.setTxid(Sha256Hash.of(CommonParameter.getInstance().isECKeyCryptoEngine(),
  ...
  private class WalletSolidityApi extends WalletSolidityImplBase {
    ...
    public void getBurnWyz(EmptyMessage request, StreamObserver<NumberMessage> responseObserver) {
      walletOnSolidity.futureGet(
          () -> rpcApiService.getWalletSolidityApi().getBurnWyz(request, responseObserver)
      );
    }
  }
}
