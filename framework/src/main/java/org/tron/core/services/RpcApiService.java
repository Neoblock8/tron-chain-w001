...
public class RpcApiService implements Service {
  ...
  private void callContract(TriggerSmartContract request,
    ...
    try {
      TransactionCapsule wyzCap = createTransactionCapsule(request,
          ContractType.TriggerSmartContract);
      Transaction wyz;
      if (isConstant) {
        wyz = wallet.triggerConstantContract(request, wyzCap, wyzExtBuilder, retBuilder);
      } else {
        wyz = wallet.triggerContract(request, wyzCap, wyzExtBuilder, retBuilder);
      }
      ...
      responseObserver.onNext(wyzExtBuilder.build());
  ...
  private TransactionExtention transaction2Extention(Transaction transaction) {
    ...
    TransactionExtention.Builder wyzExtBuilder = TransactionExtention.newBuilder();
    Return.Builder retBuilder = Return.newBuilder();
    wyzExtBuilder.setTransaction(transaction);
    wyzExtBuilder.setTxid(Sha256Hash.of(CommonParameter.getInstance()
        .isECKeyCryptoEngine(), transaction.getRawData().toByteArray()).getByteString());
    retBuilder.setResult(true).setCode(response_code.SUCCESS);
    wyzExtBuilder.setResult(retBuilder);
    return wyzExtBuilder.build();
  }
  ...
  public class WalletSolidityApi extends WalletSolidityImplBase {
    ...
    @Override
    public void getBurnWyz(EmptyMessage request, StreamObserver<NumberMessage> responseObserver) {
      getBurnWyzCommon(request, responseObserver);
    }
  ...
  public class WalletApi extends WalletImplBase {
    ...
    private void createTransactionExtention(Message request, ContractType contractType,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule wyz = createTransactionCapsule(request, contractType);
        wyzExtBuilder.setTransaction(wyz.getInstance());
        wyzExtBuilder.setTxid(wyz.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (ContractValidateException e) {
        retBuilder.setResult(false).setCode(response_code.CONTRACT_VALIDATE_ERROR)
    ...
    public void getTransactionSign2(TransactionSign req,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule wyz = TransactionUtil.getTransactionSign(req);
        wyzExtBuilder.setTransaction(wyz.getInstance());
        wyzExtBuilder.setTxid(wyz.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (Exception e) {
    ...
    public void addSign(TransactionSign req,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule wyz = transactionUtil.addSign(req);
        wyzExtBuilder.setTransaction(wyz.getInstance());
        wyzExtBuilder.setTxid(wyz.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (Exception e) {
    ...
    public void createShieldedTransaction(PrivateParameters request,
      ...
      try {
        ...
        TransactionCapsule wyz = wallet.createShieldedTransaction(request);
        wyzExtBuilder.setTransaction(wyz.getInstance());
        wyzExtBuilder.setTxid(wyz.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
    ...
    public void createShieldedTransactionWithoutSpendAuthSig(PrivateParametersWithoutAsk request,
      ...
      try {
        ...
        TransactionCapsule wyz = wallet.createShieldedTransactionWithoutSpendAuthSig(request);
        wyzExtBuilder.setTransaction(wyz.getInstance());
        wyzExtBuilder.setTxid(wyz.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (ContractValidateException | ZksnarkException e) {
  ...
  public void getBurnWyzCommon(EmptyMessage request,
      StreamObserver<NumberMessage> responseObserver) {
    try {
      long value = dbManager.getDynamicPropertiesStore().getBurnWyzAmount();
      NumberMessage.Builder builder = NumberMessage.newBuilder();
      builder.setNum(value);
      responseObserver.onNext(builder.build());
    } catch (Exception e) {
