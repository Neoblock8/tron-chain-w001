...
public class TriggerSmartContractServlet extends RateLimiterServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      TransactionCapsule wyzCap = wallet
          .createTransactionCapsule(build.build(), ContractType.TriggerSmartContract);

      Transaction.Builder txBuilder = wyzCap.getInstance().toBuilder();
      Transaction.raw.Builder rawBuilder = wyzCap.getInstance().getRawData().toBuilder();
      rawBuilder.setFeeLimit(feeLimit);
      txBuilder.setRawData(rawBuilder);

      Transaction wyz = wallet
          .triggerContract(build.build(), new TransactionCapsule(txBuilder.build()), wyzExtBuilder,
              retBuilder);
      wyz = Util.setTransactionPermissionId(jsonObject, wyz);
    ...
    response.getWriter().println(Util.printTransactionExtention(wyzExtBuilder.build(), visible));
  }
}
