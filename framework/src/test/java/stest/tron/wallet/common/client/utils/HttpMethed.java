...
public class HttpMethed {
  ...
  public static HttpResponse assetIssue(String httpNode, byte[] ownerAddress, String name,
      String abbr, Long totalSupply, Integer wyzNum, Integer num, Long startTime, Long endTime,
    ...
    try {
      ...
      userBaseObj2.addProperty("wyz_num", wyzNum);
   ...
   * constructor.
   */
  public static Long getBurnWyz(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/wallet/getburnwyz";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
    ...
    responseContent = HttpMethed.parseResponseContent(response);
    return responseContent.getLong("burnWyzAmount");
  }

  /**
   * constructor.
   */
  public static Long getBurnWyzFromSolidity(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/walletsolidity/getburnwyz";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
   ...
   * constructor.
   */
  public static Long getBurnWyzFromPbft(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/walletpbft/getburnwyz";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static HttpResponse sendShieldCoin(String httpNode, byte[] publicZenTokenOwnerAddress,
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        ...
        response = HttpMethed
            .getMerkleTreeVoucherInfo(httpNode, noteTx.getWyzId(), noteTx.getIndex(), 1);
        responseContent = HttpMethed.parseResponseContent(response);
  ...
  public static Boolean getSpendResult(String httpNode, ShieldAddressInfo shieldAddressInfo,
    ...
    try {
      ...
      jsonObjectWarp.put("note", new JSONObjectWarp().put("value", noteTx.getValue())
          .put("payment_address", noteTx.getPaymentAddress())
          .put("rcm", ByteArray.toHexString(noteTx.getR()))
          .put("memo", ByteArray.toHexString(noteTx.getMemo()))).put("txid", noteTx.getWyzId());
  ...
  public static List<ShieldNoteInfo> scanNoteByOvkFromPbft(String httpSolidityNode,
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        ...
        noteTx.setWyzId(HttpMethed.parseStringContent(noteTxs).getString("txid"));
  ...
  public static HttpResponse sendShieldCoinWithoutAsk(String httpNode, String httpSolidityNode,
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        ...
        logger.info("noteTx.getWyzId():" + noteTx.getWyzId());
        HttpMethed.response = HttpMethed
            .getMerkleTreeVoucherInfoFromSolidity(httpSolidityNode, noteTx.getWyzId(),
                noteTx.getIndex(), 1);
        ...
        HttpMethed.response = HttpMethed
            .getMerkleTreeVoucherInfoFromPbft(httpPbftNode, noteTx.getWyzId(), noteTx.getIndex(),
                1);
        HttpMethed.responseContent = HttpMethed.parseResponseContent(HttpMethed.response);
        HttpMethed.printJsonContent(responseContent);
