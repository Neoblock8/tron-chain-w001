...
public class HttpTestGetAccountBalance001 {
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Get burn wyz by http")
  public void test03GetBurnWyz() {
    Long beforeBurnWyzAmount = HttpMethed.getBurnWyz(httpnode);
    ECKey ecKey2 = new ECKey(Utils.getRandom());
    byte[] assetOwnerAddress = ecKey2.getAddress();

    HttpMethed.sendCoin(httpnode,fromAddress,assetOwnerAddress,amount,"",testKey002);
    HttpMethed.waitToProduceOneBlockFromSolidity(httpnode,httpSolidityNode);
    Long afterBurnWyzAmount = HttpMethed.getBurnWyz(httpnode);
    Assert.assertTrue(afterBurnWyzAmount - beforeBurnWyzAmount == 100000L);

    Assert.assertEquals(afterBurnWyzAmount,HttpMethed.getBurnWyzFromSolidity(httpSolidityNode));
    Assert.assertEquals(afterBurnWyzAmount,HttpMethed.getBurnWyzFromPbft(httpPbftNode));
  }
