...
public class MarketPairPriceToOrderStoreWithCacheOldValueTest {
  ...
  public synchronized void testGetKeysNext() {
    ...
    TestRevokingTronStore WyzthDatabase = new TestRevokingTronStore(dbName, options,
        revokingDatabase);
    ...
    // put: 2 1 0 3
    Assert.assertFalse(WyzthDatabase.has(pairPriceKey2));
    WyzthDatabase.put(pairPriceKey2, capsule2);
    ...
    Assert.assertTrue(WyzthDatabase.has(pairPriceKey1));
    WyzthDatabase.put(pairPriceKey1, capsule1);
    Assert.assertEquals(1, WyzthDatabase.size());
    ...
    Assert.assertFalse(WyzthDatabase.has(pairPriceKey0));
    if (!WyzthDatabase.has(pairPriceKey0)) {
      WyzthDatabase.put(pairPriceKey0, capsule0);
    }

    Assert.assertEquals(2, WyzthDatabase.size());

    Assert.assertFalse(WyzthDatabase.has(pairPriceKey3));
    if (!WyzthDatabase.has(pairPriceKey3)) {
      WyzthDatabase.put(pairPriceKey3, capsule3);
    }

    Assert.assertEquals(3, WyzthDatabase.size());

    // get pairPriceKey1, will get pairPriceKey2's value capsule2
    try {
      Assert
          .assertArrayEquals(capsule0.getData(),
              WyzthDatabase.get(pairPriceKey0).getData());
      Assert
          .assertArrayEquals(capsule1.getData(),
              WyzthDatabase.get(pairPriceKey1).getData());
      Assert
      ...
      Assert
          .assertArrayEquals(capsule3.getData(),
              WyzthDatabase.get(pairPriceKey3).getData());
    } catch (ItemNotFoundException | BadItemException e) {
      Assert.fail();
    }

    List<byte[]> keyList = WyzthDatabase.getRevokingDB().getKeysNext(pairPriceKey0, 2 + 1);
  ...
  private static class TestRevokingTronStore extends
      WyzthStoreWithRevoking<MarketOrderIdListCapsule> {
