...
contract D {
  ...
  public void testAAfterAllowMultiSignProposal()
    ...
    this.manager.getDynamicPropertiesStore().saveAllowWvmTransferTrc10(1);
    ...
    Runtime aRuntime = WvmTestUtils
        .processTransactionAndReturnRuntime(aWyz, DepositImpl.createRoot(manager), null);
    ...
    Transaction cWyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        "testC", address, abi, cCode, value, fee, consumeUserResourcePercent, null, engeryLimit);
    Runtime cRuntime = WvmTestUtils
        .processTransactionAndReturnRuntime(cWyz, DepositImpl.createRoot(manager), null);
    Assert.assertTrue(cRuntime.getResult().getException() instanceof OutOfEnergyException);

    Transaction dWyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        "testC", address, abi, dCode, value, fee, consumeUserResourcePercent, null, engeryLimit);
    Runtime dRuntime = WvmTestUtils
        .processTransactionAndReturnRuntime(dWyz, DepositImpl.createRoot(manager), null);
