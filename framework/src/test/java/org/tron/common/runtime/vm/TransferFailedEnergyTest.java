...
public class TransferFailedEnergyTest extends VMTestBase {
  ...
  contract EnergyOfTransferFailedTest {
      ...
      // InsufficientBalance
      function testTransferWyzInsufficientBalance() payable public{
          msg.sender.transfer(10);
      }

      function testSendWyzInsufficientBalance() payable public{
          msg.sender.send(10);
      }
      ...
      function testCallWyzInsufficientBalance(address payable caller) public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }

      function testCreateWyzInsufficientBalance() payable public {
          (new Caller).value(10)();
      }

      // NonexistentTarget
      function testTransferWyzNonexistentTarget(address payable nonexistentTarget) payable public {
      ...
      function testCallWyzNonexistentTarget(address payable nonexistentTarget) payable public {
          require(address(this).balance >= 10);
      ...
      // target is self
      function testTransferWyzSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.transfer(10);
      }

      function testSendWyzSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.send(10);
      }
  ...
  contract EnergyOfTransferFailedTest {
      ...
      function testCallWyzInsufficientBalance(address caller) payable public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }
      ...
      // NonexistentTarget
      function testTransferWyzNonexistentTarget(address nonexistentTarget) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transfer(10);
      }
      ...
      function testCallWyzNonexistentTarget(address nonexistentTarget) public {
          require(address(this).balance >= 10);
          nonexistentTarget.call.value(10)(abi.encodeWithSignature("test()"));
      }
  ...
  */
  ...
  TestCase[] testCasesAfterAllowWvmConstantinop = {
      new TestCase("testTransferWyzSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSendWyzSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSuicideNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testTransferWyzNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testCallWyzNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
  };
  TestCase[] testCasesBeforeAllowWvmConstantinop = {
      new TestCase("testTransferWyzSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSendWyzSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSuicideNonexistentTarget(address)",
  ...
  TestCase[] testCasesInsufficientBalance = {
      new TestCase("testTransferWyzInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testSendWyzInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.SUCCESS),
      new TestCase("testCreateWyzInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testCallWyzInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
  ...
  @Test
  public void testTransferFailedAfterAllowWvmConstantinopl()
    ...
    manager.getDynamicPropertiesStore().saveAllowWvmSolidity059(0);
    ...
    Transaction wyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] addressWithSufficientBalance = WalletUtil.generateContractAddress(wyz);
    runtime = WvmTestUtils.processTransactionAndReturnRuntime(wyz, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());

    for (TestCase testCase : testCasesAfterAllowWvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }

    wyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, 0, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithoutBalance = WalletUtil.generateContractAddress(wyz);
    runtime = WvmTestUtils.processTransactionAndReturnRuntime(wyz, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
  ...
  @Test
  public void testTransferFailedBeforeAllowWvmConstantinopl()
      throws ContractExeException, ReceiptCheckErrException,
      VMIllegalException, ContractValidateException {
    ConfigLoader.disable = false;
    manager.getDynamicPropertiesStore().saveAllowWvmTransferTrc10(1);
    manager.getDynamicPropertiesStore().saveAllowWvmConstantinople(0);
    manager.getDynamicPropertiesStore().saveAllowWvmSolidity059(0);
    ...
    for (TestCase testCase : testCasesBeforeAllowWvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }
