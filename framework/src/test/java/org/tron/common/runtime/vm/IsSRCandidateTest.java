...
public class IsSRCandidateTest extends VMTestBase {
  ...
  */
  ...
  public void testIsSRCandidate()
    ...
    // deploy contract
    Transaction wyz =
        WvmTestUtils.generateDeploySmartContractAndGetTransaction(
    ...
    wyz =
        WvmTestUtils.generateTriggerSmartContractAndGetTransaction(
            Hex.decode(OWNER_ADDRESS), factoryAddress, Hex.decode(hexInput), 0, fee);
    InternalTransaction rootInternalTransaction =
        new InternalTransaction(wyz, InternalTransaction.WyzType.WYZ_CONTRACT_CALL_TYPE);
