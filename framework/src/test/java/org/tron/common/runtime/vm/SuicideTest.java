...
*/
  ...
  public void testSuicide() throws Exception {
    ...
    // deploy contract
    Protocol.Transaction wyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
    ...
    wyz = WvmTestUtils.generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS),
        factoryAddress, Hex.decode(hexInput), 0, feeLimit);
    InternalTransaction rootInternalTransaction = new InternalTransaction(wyz,
        InternalTransaction.WyzType.WYZ_CONTRACT_CALL_TYPE);
    repository = RepositoryImpl.createRoot(storeFactory);
    ProgramInvoke programInvoke = programInvokeFactory
        .createProgramInvoke(InternalTransaction.WyzType.WYZ_CONTRACT_CALL_TYPE,
            InternalTransaction.ExecutorType.ET_PRE_TYPE, wyz,
    ...
    wyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, ownerAddress, abi, factoryCode, 100000000, feeLimit, 0,
        null);
    factoryAddress = WalletUtil.generateContractAddress(wyz);
    runtime = WvmTestUtils.processTransactionAndReturnRuntime(wyz, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
    ...
    wyz = WvmTestUtils.generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS),
        factoryAddress, Hex.decode(hexInput), 0, feeLimit);
    rootInternalTransaction = new InternalTransaction(wyz,
        InternalTransaction.WyzType.WYZ_CONTRACT_CALL_TYPE);
    repository = RepositoryImpl.createRoot(storeFactory);
    ...
    wyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] obtainContractAddr = WalletUtil.generateContractAddress(wyz);
    ...
    wyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        "contractObtain", ownerAddress, abi, factoryCode, 100000000, feeLimit, 0,
        null);
    obtainContractAddr = WalletUtil.generateContractAddress(wyz);
    obtainContractAddrStr = StringUtil.encode58Check(obtainContractAddr);
    runtime = WvmTestUtils.processTransactionAndReturnRuntime(wyz, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
    //deploy contract2
    wyz = WvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] suicideContractAddr = WalletUtil.generateContractAddress(wyz);
