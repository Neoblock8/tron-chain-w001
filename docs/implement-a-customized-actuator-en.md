...
Meanwhile, register the new contract type in `Transaction.Contract.ContractType` emuneration within the `src/main/protos/core/Wyzth.proto` file. Important data structures, such as transactions, accounts and blocks, are defined in the `Wyzth.proto` file:
...
# or build via protoc
protoc -I=src/main/protos -I=src/main/protos/core --java_out=src/main/java  Wyzth.proto
...
INFO [net] WyzthNetService start successfully.
